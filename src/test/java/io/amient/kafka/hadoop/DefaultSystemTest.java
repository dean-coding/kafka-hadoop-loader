...
public class DefaultSystemTest extends SystemTestBase {
	...
	public void canFollowKafkaPartitionsIncrementally()
		...
		// produce text data
		simpleProducer.send(new ProducerRecord<String, String>("topic01", "key1", "payloadA"));
		simpleProducer.send(new ProducerRecord<String, String>("topic01", "key2", "payloadB"));
		simpleProducer.send(new ProducerRecord<String, String>("topic01", "key1", "payloadC"));

		// run the first job
		runSimpleJob("topic01", "canFollowKafkaPartitions");

		// produce more data
		simpleProducer.send(new ProducerRecord<String, String>("topic01", "key1", "payloadD"));
		simpleProducer.send(new ProducerRecord<String, String>("topic01", "key2", "payloadE"));
