...
public class KafkaInputFormat extends InputFormat<MsgMetadataWritable, BytesWritable> {
	...
	// TODO #12 instead of the following settings pass anything kafka.consumer.* to
	...
	public List<InputSplit> getSplits(JobContext context) throws IOException, InterruptedException {
		...
		try (KafkaZkUtils zkUtils = new KafkaZkUtils(conf.get(KafkaInputFormat.CONFIG_ZK_CONNECT),
	...
	@Override
	public RecordReader<MsgMetadataWritable, BytesWritable> createRecordReader(InputSplit inputSplit,
	...
	private List<InputSplit> createSplitsForPartitionLeader(Configuration conf, KafkaZkUtils zk) throws IOException {
		...
		for (String topic : inputTopics) {
			...
			for (int partition : partitionLeaders.keySet()) {
				...
				KafkaInputSplit split = new KafkaInputSplit(brokerId, zk.getBrokerName(brokerId), topic, partition,
